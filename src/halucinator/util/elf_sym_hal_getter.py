...
def load_binary(filename):
    ...
    loader = angr.cle.loader.Loader(filename, auto_load_libs=False, use_system_libs=False)
    return loader
...
def main():
    ...
    p = ArgumentParser()
    p.add_argument("-b", "--bin", required=True, help="Elf file to get symbols from")
    p.add_argument(
        "-o",
        "--out",
        required=False,
        help="YAML file to save output to" + "if will be output to (--bin).yaml",
    )
