...
import yaml
from avatar2 import ARM_CORTEX_M3, Avatar, GDBTarget
import IPython
...
class State_Recorder(object):
    ...
    def create_sql_tables(self, db):
        ...
        cursor = db.cursor()
        cursor.execute(
            "CREATE TABLE IF NOT EXISTS applications (id INTEGER PRIMARY KEY, name TEXT, "
            + "sha1 TEXT, bin BLOB)"
        )
        cursor.execute(
            """CREATE TABLE IF NOT EXISTS states (id INTEGER PRIMARY KEY,
                            app_id INTEGER,
                            function_name TEXT,
    ...
    def get_app_id(self, elf_file, db):
        ...
        if row is None:
            c.execute(
                "INSERT INTO applications(name, sha1, bin) VALUES(?,?,?)",
    ...
    def save_state_to_db(self, function, is_entry):
        ...
        if is_entry:
            c.execute(
                """INSERT INTO states (app_id, function_name,
                         memory, regs) VALUES(?,?,?,?)""",
                (self.app_id, function, mem, regs),
            )
        else:
            ...
            c.execute(
                """INSERT INTO states (app_id, function_name,
                         memory, regs, entry_id) VALUES (?,?,?,?,?)""",
                (self.app_id, function, mem, regs, entry_id),
            )
        db.commit()
...
if __name__ == "__main__":
    ...
    p.add_argument("-f", "--functions", required=True, help="YAML file listing functions")
    p.add_argument("-d", "--db", help="sqlite3 database filename")
    args = p.parse_args()

    avatar = Avatar(arch=ARM_CORTEX_M3, output_directory="/tmp/hal_profile")
    gdb = avatar.add_target(
        GDBTarget, gdb_additional_args=[args.elf], gdb_executable="arm-none-eabi-gdb", gdb_port=2331
    )

    avatar.watchmen.add_watchman("BreakpointHit", "before", handle_bp, is_async=True)
    avatar.init_targets()
