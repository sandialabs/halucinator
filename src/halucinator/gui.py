...
class App(object):
    ...
    def get_exit_records(self):
        ...
        for row in c.execute(
            "SELECT id, function_name, entry_id, app_id FROM states WHERE entry_id NOT NULL"
        ):
    ...
    def create_gui_elements(self):
        ...
        self.record_tree = tkinter.ttk.Treeview(
            master=self.root, columns=("Type", "Name", "Size", "Before", "After")
        )
    ...
    def display_recording(self, record):
        ...
        ele = self.record_tree.insert("", 0, iid=funct_die.offset, values=values)
    ...
    def add_children(self, record, sym_reader, sym_die, parent_values, parent, deref_addrs):
        """
        ...
        if "DW_AT_type" in sym_die.attributes:
            ...
            if type_die.tag == "DW_TAG_pointer_type":
                ...
                if b_ptr not in deref_addrs["before"] or a_ptr not in deref_addrs["after"]:
                    ...
                    self.add_children(record, sym_reader, type_die, parent_values, row, deref_addrs)
                else:
                    values = (ty_str, "", ty_size, "Already Derefed", "Already Derefed")
                    row = self.record_tree.insert(parent, tk.END, values=values)

            elif type_die.tag == "DW_TAG_const_type":
                self.add_children(record, sym_reader, type_die, parent_values, parent, deref_addrs)
                pass
                ...
                row = self.add_record_row(parent, sym_reader, type_die, None, None, None)
                ...
                for child in type_die.iter_children():
                    ...
                    row = self.add_record_row(parent, sym_reader, child, child_name, b_val, a_val)
                    parent_vals = {"before": b_val, "after": a_val}
                    self.add_children(record, sym_reader, child, parent_vals, row, deref_addrs)
...
if __name__ == "__main__":
    ...
    p = ArgumentParser()
    p.add_argument("-d", "--database", required=True, help="Recorded Database")
