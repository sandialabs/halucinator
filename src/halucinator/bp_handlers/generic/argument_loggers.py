...
class ArgumentLogger(BPHandler):
    ...
    def register_handler(
        ...
        log.debug(
            "Registration Args: "
            + "Fun: %s, num_args %i, log_ret_addr %s intercept %s, ret_value %s, silent %s "
            % (func_name, num_args, log_ret_addr, intercept, ret_value_str, silent)
        )
        self.loggers[addr] = ArgumentLogger.Logger(
            target, func_name, num_args, log_ret_addr, intercept, ret_value, silent
        )
        return ArgumentLogger.log_handler

    class Logger:
        def __init__(self, target, func_name, num_args, log_caller, intercept, ret_value, silent):
