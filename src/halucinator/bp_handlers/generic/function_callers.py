...
from halucinator.bp_handlers.bp_handler import bp_handler
...
class FunctionCaller:
    def __init__(self, qemu, start_addr, size, callee_addr, args, callee_fname=None):
        self.qemu = qemu
        self.args = args
...
class FunctionCallerIntercept:
        ...
        for mem_interval in self.qemu.avatar.memory_ranges:
            ...
            if m.name == self.MEMORY_REGION_NAME:
                if m.permissions != "rwx":
                    raise (ValueError("Memory region %s must be 'rwx'" % self.MEMORY_REGION_NAME))
                return m.address, m.size
        raise (
            ValueError(
                "Memory Region named: %s required by %s" % (self.MEMORY_REGION_NAME, self.__class__)
            )
        )
    ...
    def register_handler(
        ...
        **Note** only a single instance of the class is create, and this
        ...
        if self.qemu.avatar.arch != avatar2.archs.arm.ARM:
            raise (ValueError("Architecture (%s) not supported") % (str(self.qemu.avatar.arch)))
        ...
        if type(callee) == str:
            try:
                ...
                log.error(
                    "Callee(%s) invalid for %s, 0x%08x, check intercept config"
        ...
        if self.qemu.avatar.arch == avatar2.archs.arm.ARM:
            ...
            caller = ARMFunctionCaller(
                self.qemu, stack_addr, stack_size, callee_addr, args, callee_fname
            )
            ...
            if watchpoint == "":
                ...
                self.setup_return_bp(
                    function, callee_addr, return_addr, break_type="WP", rw=watchpoint
                )
    ...
    def get_stack_addr(self, size):
        ...
        raise (
            ValueError(
                "Insufficient Memory for stacks "
                + "increase size of %s memory regions" % FunctionCallerIntercept.MEMORY_REGION_NAME
            )
        )
    ...
    def setup_return_bp(self, function, callee_addr, return_addr, break_type="BP", rw="r"):
        if break_type == "WP":
            config = {
                ...
                "registration_args": {"callee": callee_addr, "is_return": True},
                "run_once": True,
