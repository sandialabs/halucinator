...
class Ethernet(BPHandler):
    ...
    def register_handler(self, qemu, addr, func_name,
                        end_handle_rcv=None, net_pool_offset=None,
        ...
        self.net_pool_offset = net_pool_offset if net_pool_offset is not None \
            else self.net_pool_offset
        self.cl_pool_id_offset = cl_pool_id_offset if cl_pool_id_offset is not None \
            else self.cl_pool_id_offset

        if end_handle_rcv is not None:
            self.handle_end_int_rcv_addr = qemu.avatar.config.get_addr_for_symbol(end_handle_rcv)
            if self.handle_end_int_rcv_addr is None:
                raise ValueError("Could not find address for end_handle_rcv  %s"% end_handle_rcv)

        return super().register_handler(qemu, addr, func_name)

    @bp_handler(['netPoolInit'])
    def net_pool_init(self, qemu, addr):
        '''
            net_pool_init
        '''
        with open("NetPools.txt", "a") as outfile:
            callee = qemu.avatar.config.get_symbol_name(qemu.regs.lr)
            outfile.write("Netpool: %#x, called from %s\n"%(qemu.regs.r0, callee))
        return False, None


    def get_eth_id(self, qemu):
        '''
            get_eth_id
        '''
    ...
    def e_io_cs_addr(self,qemu,p_obj,ptr_mac_addr):
        '''
            EIOCGADDR
        '''
    ...
    def e_io_cg_addr(self,qemu,p_obj,ptr_mac_addr):
        '''
            EIOCSADDR
        '''
    ...
    def e_io_cs_flags(self,qemu,p_obj,ptr_mac_addr):
        '''
            EIOCSFLAGS
        '''
    ...
    def get_packetdata(self, qemu,mblk):
        ''' 
        '''
        mblk_out ={}
        mblk_out['addr']=hex(mblk)
        mblk_out["mBlkHdr"] = {}
        mblk_out["p_cl_blk"] = qemu.read_memory((mblk + 0x30), 4, 1,)
        ...
        mblk_out["mBlkHdr"]["PKT_DATA"] = qemu.read_memory(
                                            mblk_out["mBlkHdr"]["m_data"], 1,
                                            mblk_out["mBlkHdr"]["m_len"], raw=True)
        return mblk_out

    @bp_handler(['xSend'])
    def x_send(self, qemu, handler):
        '''
    ...
    @bp_handler(['xUnload'])
    def x_unload(self, qemu, handler):
        '''
            xUnload bp_handler
        '''
        log.debug("x_unload")
    ...
    def x_poll_send(self, qemu, handler):
        '''
    ...
    def x_m_cast_addr_del(self, qemu, handler):
        '''
    ...
    def x_m_cast_addr_get(self, qemu, handler):
        '''
    ...
    def x_m_cast_addr_add(self, qemu, handler):
        '''
    ...
    def ethernet_isr(self, qemu, bp_addr):
        '''
        ...
        return qemu.call('netJobAdd',[self.handle_end_int_rcv_addr, qemu.get_arg(0),0,0,0,0])

    @bp_handler(['handleEndIntRcv'])
    def handle_end_int_rcv(self, qemu, handler):
        '''handle_end_int_rcv'''
        ...
        return qemu.call('netTupleGet',[self.p_net_pool, 0x5f0, 1, 1],self, 'netTupleGet_return')

    def set_mblk(self, qemu, mblk_addr, data, flags=3, pk_hdr=42):
        '''set_mblk'''
        p_cl_blk = qemu.read_memory((mblk_addr + 0x30), 4, 1,)
    ...
    def get_mblk(self, qemu, handler):
        '''get_mblk'''
        self.m_blk = qemu.regs.r0
        if self.m_blk == 0:
            raise TypeError("Failed to get netTuple")
        frame = self.eth_model.get_rx_frame(self.get_eth_id(qemu))
        self.set_mblk(qemu, self.m_blk, frame)
        return qemu.call('muxReceive',[self.p_dev, self.m_blk], self, 'receive_done')

    @bp_handler(['call_muxReceive'])
    def call_mux_receive(self, qemu, handler):
        '''call_mux_receive'''
        eth_id = self.get_eth_id(qemu)
    ...
    def receive_done(self, qemu, handler):
        '''receive_done'''
        log.debug("DONE With MuxReceive ....................................................")
        eth_id = self.get_eth_id(qemu)
        self.eth_model.enable_rx_isr_bp(eth_id)
        return True, None
