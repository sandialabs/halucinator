...
class TYDev(BPHandler):
    ...
    '''
    def __init__(self, model=UTTYModel, tty_dev_offset=0x10, sema_ptr_offset=0x650,
                 interfaces=None, ird='tyIRd', use_rx_task=False):
        super().__init__()
    ...
    def ty_isr(self, qemu, handler):
        ...
        tty_dev_struct = int.from_bytes(
            qemu.read_memory((p_ty_dev + self.tty_dev_offset), 1, 4, raw=True),
            "little")
        ...
        if sema_val == 0 or not self.use_rx_task:
            ...
            if num_chars_rx >0:
                ...
                # Add State to stack to ensure the correct state is retreived when isr_execute_read is called
    ...
    def rx_task(self, qemu, handler):
        ...
        tty_dev_struct = int.from_bytes(
            qemu.read_memory((qemu.get_arg(0) + 0x10), 1, 4, raw=True),
            "little")
        isr_state =  TYIsrState(tty_dev_struct,dev_id,num_chars_rx)
    ...
    def fio_rflush(self,qemu,p_obj,arg):
        ...
        p_ty_dev = qemu.get_arg(0)
        dev_id = self.get_utty_id(qemu, p_ty_dev)
        buf_size = self.utty_model.flush(dev_id)
        return True, 0
