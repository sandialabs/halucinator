...
INCLUDES = [
    "-I" + path.join(STM_DRIVER_PATH, "STM32F4xx_HAL_Driver/Inc"),
    "-I" + path.join(STM_DRIVER_PATH, "CMSIS/Include/"),
    "-I" + path.join(STM_DRIVER_PATH, "CMSIS/Device/ST/STM32F4xx/Include/"),
    "-I"
    + path.expanduser(
        "~/projects/HALucinator/avatar-devices/3rd_party/pycparser-release_v2.18/"
...
def remove_unsupported_code(infile, outfile):
    ...
    Removes C language features not supported by pycparser:
        ...
        __asm
    """
    remove_regexs = [r"__attribute__\s*\(\s*\(\s*\S*\s*\)\s*\)", r"__asm.*\);"]  # __asm directives
...
if __name__ == "__main__":
    ...
    p = ArgumentParser()
    p.add_argument("-d", "--indir", required=True, help="Directory of header files to preprocess")
    p.add_argument(
        "-o", "--outdir", required=True, help="Name of directory to write preprocessed files to"
    )
    args = p.parse_args()
