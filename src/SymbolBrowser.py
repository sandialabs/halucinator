...
class App(object):
    ...
    def create_recording_listing(self):
        ...
        for f_name in sorted(self.dwarf_reader.function_lut.keys()):
            _ = self.tree.insert("", tk.END, iid=f_name, text=f_name)  # f_row
    ...
    def add_children(self, funct_name, sym_reader, sym_die, parent_values, parent, deref_addrs):
        """
        ...
        if "DW_AT_type" in sym_die.attributes:
            ...
            if type_die.tag == "DW_TAG_pointer_type":
                ...
                if b_ptr not in deref_addrs["before"] or a_ptr not in deref_addrs["after"]:
                    ...
                    self.add_children(
                        funct_name, sym_reader, type_die, parent_values, row, deref_addrs
                    )
                else:
                ...
                self.add_children(
                    funct_name, sym_reader, type_die, parent_values, parent, deref_addrs
                )
                pass
                ...
                for child in type_die.iter_children():
                    ...
                    self.add_children(funct_name, sym_reader, child, parent_vals, row, deref_addrs)
...
if __name__ == "__main__":
    ...
    p = ArgumentParser()
    p.add_argument("-e", "--elf", required=True, help="elf file")
