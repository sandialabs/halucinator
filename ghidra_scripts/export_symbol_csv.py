...
def print_program_info(currentProgram):
  ...
  print "Program Info:\tbinary name: %s\t\tArchitecture_Compiler: \
          %s_%s\tCreation Date:(%s)\n" % (program_name, language_id, \
          compiler_spec_id, creation_date)
  ...
  with open(filename, "w") as outfile:
    ...
    while function is not None:
      ...
      if "task" in name:
        ...
        endAddr = str(hex(int(function.getEntryPoint().toString(), 16) + function.getBody().getNumAddresses())).rstrip("L")
        ...
        outfile.write(" - class: halucinator.bp_handlers.generic.armv7m_param_log.ARMv7MEABILogger\n   registration_args: {ret_val: null, no_intercept: true, lr: true}\n   addr:  %s\n   function: %s\n" %(startAddr, name))
      function = getFunctionAfter(function)
        # print_program_info(currentProgram)
  numSymbols = currentProgram.getSymbolTable().getNumSymbols()
  monitor.initialize(numSymbols)
  filename = askString("Symbol Table CSV Output File Name", "Enter Full Path for where to save Symbol Table Output File Name")
  with open(filename, "w") as outfile:
